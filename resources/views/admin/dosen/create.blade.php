...
<div class="bg-bg-card border border-border-custom rounded-2xl overflow-hidden shadow-xs max-w-[640px]">
    <div class="px-6 py-4 border-b border-border-custom flex items-center justify-between">
        <h2 class="text-[0.95rem] font-bold text-text-primary flex items-center gap-2"><i class="bi bi-person-plus-fill text-accent"></i> Form Tambah Dosen</h2>
    </div>
    <div class="p-6">
        <form method="POST" action="{{ route('admin.dosen.store') }}">
            @csrf
            <div class="mb-5">
                <label for="nidn" class="block text-[0.82rem] font-semibold mb-1.5 text-text-muted">NIDN <span class="text-red-500">*</span></label>
                <input id="nidn" type="text" name="nidn" maxlength="10"
                       class="w-full px-3.5 py-2.5 bg-white border rounded-lg text-text-primary text-[0.875rem] outline-none transition-all duration-200 focus:border-accent focus:ring-3 focus:ring-accent/10 @error('nidn') border-danger-custom focus:ring-red-500/10 @else border-border-custom @enderror"
